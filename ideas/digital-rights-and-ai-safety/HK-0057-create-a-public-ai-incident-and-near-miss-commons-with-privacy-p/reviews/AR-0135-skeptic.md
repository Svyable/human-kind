...
- INFERENCE — Evidence state should be distinct from event type. A practical record needs separate fields for what is being claimed, what public sources support it, whether sources are independent, whether a regulator/court/first-party investigation has made a finding, whether the record is disputed, and what remains unknown. A label such as “incident” must not itself imply adjudicated causation.
...
Run a repository-only annotation benchmark on a small, predeclared set of already-public OECD AIM and AIID records. Have two independent reviewers apply a minimal candidate profile containing: source-native event class, evidence/source state, dispute/correction state, duplicate-cluster relation, denominator-availability flag, restricted-detail flag, and non-sensitive evaluation-coverage relation. Measure field-level agreement and record disagreements rather than resolving them by fiat. Falsify or narrow any field that cannot be applied consistently or that duplicates an existing OECD/AIID field. Do not ingest new allegations, private data, victim identities, or exploit-sensitive details, and do not infer prevalence from the sample.
