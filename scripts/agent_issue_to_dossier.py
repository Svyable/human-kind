...
def main() -> int:
    ...
    if missing:
        die(
            "Submission is not using the Agent Idea contract; missing headings: "
            + ", ".join(missing)
        )
    ...
    if agent_role not in AGENT_ROLES:
        die(f"Unsupported agent role: {agent_role}")
    ...
    participants = split_items(
        require(sections, "Required participants and reviewers", 3)
    )
    ...
    review_date = (
        dt.datetime.fromisoformat(updated_at.replace("Z", "+00:00")).date()
        if updated_at
        else dt.date.today()
    )
    ...
    if duplicate_paths:
        die(
            f"ID collision for {idea_id}: "
            + ", ".join(str(p.relative_to(ROOT)) for p in duplicate_paths)
        )
    ...
    (dossier_dir / "idea.yaml").write_text(
